# =============================================================================
# IANC H2O V2.0 - AUDITORÍA MULTI-SENSOR Y GRADIENTE HIDRÁULICO
...
st.set_page_config(page_title="IANC H2O - Auditoría Multi-Sensor", layout="wide")
...
st.title("📡 SISTEMA INTEGRAL DE AUDITORÍA IANC H2O")
st.caption(f"Propiedad Intelectual: {AUTOR} | Análisis de Gradiente Multi-Punto")

# --- SECCIÓN DE MAPA (COLOCACIÓN DE N SENSORES) ---
...
with col_inputs:
    ...
    if not st.session_state.puntos:
        st.info("Haga clic en el mapa para añadir sensores.")
    
    # Generación dinámica de entradas para cada sensor colocado
    ...
    if st.button("🗑️ Reiniciar Auditoría"):
        st.session_state.puntos = []
        st.session_state.datos_sensores = {}
        st.rerun()

# =================================================================
# MOTOR DE CÁLCULO Y GRADIENTE HIDRÁULICO (AUTOMÁTICO)
# =================================================================
if len(st.session_state.puntos) >= 2:
    ...
    for i in range(len(st.session_state.puntos)):
        ...
        if i > 0:
            ...
            # Balance de Energía en el tramo
        ...
        grafico_h.append({"Distancia": dist_acumulada, "Energía H": energia_h, "Cota Z": datos_act['Z']})

    # Visualización del Gradiente (Gráfico)
    df_grafico = pd.DataFrame(grafico_h).set_index("Distancia")
    st.line_chart(df_grafico)

    # --- TABLA AUTOMÁTICA DE RESULTADOS (INFERIOR) ---
    st.subheader("📋 Matriz de Auditoría y Estado de Tubería")
    df_resumen = pd.DataFrame(tabla_final)
    st.table(df_resumen)

    # Alerta de Fugas Localizadas
    if fugas_encontradas:
        for f in fugas_encontradas:
            st.error(f"🚨 RUPTURA DETECTADA en {f['Tramo']}: Se pierden **{f['Caudal']:.2f} L/s** a los **{f['Distancia']:.1f} m** del origen.")
    else:
        st.success("✅ Integridad confirmada en todos los tramos analizados.")
